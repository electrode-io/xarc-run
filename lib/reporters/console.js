...
class XReporterConsole {
  ...
  _onExeType_shell(data) {
    ...
    const msg = data.anon
      ...
      : `Executing ${chalk.cyan(qItem.name)}${this._depMsg(qItem)} ${chalk.blue(data.cmd)}`;
    this._log(qItem, msg);
  }
  ...
  _onExeType_serial_arr(data) {
    ...
    const msg = `Processing ${chalk.cyan(qItem.name)}${this._depMsg(qItem)} serial array ${ts}`;
    this._log(qItem, msg);
  }
  ...
  _onExeType_concurrent_arr(data) {
    const qItem = data.qItem;
    const ts = chalk.blue(stringify(data.array));
    const msg = `Processing ${chalk.cyan(qItem.name)}${this._depMsg(qItem)} concurrent array ${ts}`;
    this._log(qItem, msg);
  }
  ...
  _onExeType_function(data) {
    ...
    const msg = `Executing ${name}${this._depMsg(qItem)}${anon}function`;
    this._log(qItem, msg);
  }
  ...
  _onDoneItem(data) {
    ...
    const msec = `(${data.elapse.toFixed(3)} ms)`;
