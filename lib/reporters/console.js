...
class XReporterConsole {
  ...
  _onExeType_env(data) {
    ...
    const msg = data.anon ? `Setting ${str}` : `Execute ${name} setting ${chalk.blue(str)}`;
    this._log(qItem, msg);
  }
  ...
  _onExeType_shell(data) {
    ...
    if (data.cmdVal.constructor.name === "XTaskSpec") {
      cmd = xsh.pathCwd.replace(data.cmdVal.toString(), null, "g");
    } else {
      cmd = xsh.pathCwdNm.replace(data.cmd, null, "g");
    }
    const msg = data.anon
  ...
  _onExeType_concurrent_arr(data) {
    const qItem = data.qItem;
    const name = this._itemDisplayName(qItem);
    const ts = chalk.blue(xsh.pathCwd.replace(stringify(data.array), null, "g"));
