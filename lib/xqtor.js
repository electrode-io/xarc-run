...
class XQtor {
  ...
  execute() {
    ...
    if (vtype === "String") {
      ...
      if (value.item.dep && !qItem.xqDep) {
        logger.log(qItem.indent() + `Processing task ${chalk.cyan(qItem.name)} dependencies`);
        qItem.xqDep = true;
        this._enqueue(qItem);
        this._insertDone(qItem, `processing task ${chalk.cyan(qItem.name)} dependencies`);
        this._enqueue(
  ...
  _processTaskObject(qItem) {
    ...
    if (type === "Array") {
      ...
      logger.log(qItem.indent() + `Executing task ${chalk.cyan(qItem.name)} ${chalk.blue(itemTask)}`);
  ...
  _processSerialArray(qItem, tasks, slice) {
    ...
    logger.log(qItem.indent() + `Processing task ${chalk.cyan(taskName)} serial array ${ts}`);
    if (slice) {
      tasks = tasks.slice(1);
  ...
  _processConcurrentArray(qItem, tasks) {
    const taskName = qItem.name || this._parentName(qItem);
    const ts = chalk.blue(this._stringifyArray(tasks));
    logger.log(qItem.indent() + chalk.magenta(`Processing task ${chalk.cyan(taskName)} concurrent array ${ts}`));
  ...
  _processDone(qItem) {
    ...
    logger.log(`${xqItem.indent()}${status} ${value.msg} ${chalk.magenta(msec)}`);
    this.next();
  }
