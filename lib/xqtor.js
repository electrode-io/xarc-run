...
class XQtor {
  ...
  _processDep(qItem, dep) {
    ...
    const di = this._createQItem(
      {
        name: `${qItem.name}-dep`,
        value: { top: true, item: { task: dep } }
      },
      this._clap.xqTree.parent(qItem)
    );

    // If it's not an array, then it should be a string or function, which
    // will be executed directly in next cycle so need to add a done mark for it
    if (!Array.isArray(dep)) {
      const tof = typeof dep;
      assert(tof === "string" || tof === "function", `${qItem.name} dep type ${tof} is not string or function`);
      this._insertMark(di);
    }
    this._enqueue(di);
  ...
  _processTaskObject(qItem) {
    ...
    if (type === "Array") {
      ...
      return exec(itemTask, false, err => this.next(err));
    } else {
  ...
  _processArray(qItem, tasks, top) {
    ...
    if (top || ss) {
      this._processSerialArray(qItem, tasks, top, ss);
    } else {
      this._processConcurrentArray(qItem, tasks);
    }
  }

  _processSerialArray(qItem, tasks, top, slice) {
    this._emit("execute", {
      type: "serial-arr",
      qItem,
      array: tasks
  ...
  _processMoreFromFn(qItem, value, callback) {
    ...
    if (callback) {
      assert(typeof callback === "function", `${qItem.name} callback from function calling run is not a function`);
