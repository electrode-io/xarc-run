...
class XQtor {
  ...
  execute() {
    ...
    this._clap.fail(
      new Error(
        `Unable to process task ${qItem.name} because value type ${vtype} is unknown and no value.item`
      )
    );
    ...
    this.queue = tasks
      .map(value => {
        return this._createQItem(
          this._resolveValueToQItemOptions(`${qItem.name}.S`, value, "serial_child"),
          qItem
        );
      })
      .concat(this.queue);
