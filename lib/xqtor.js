...
class XQtor {
  ...
  execute() {
    ...
    if (vtype === "String") {
      if (this._isAnonShell(value)) {
        ...
        return this._shellXer(qItem, cmd, true);
      } else {
  ...
  _concurrentArrayXer(qItem, tasks) {
    ...
    Insync.parallel(
      tasks.map(value => cb => {
        ...
        const options = this._resolveValueToQItemOptions(`${qItem.name}.C`, value, "concurrent_child");
  ...
  _shellXer(qItem, cmd, anon) {
    ...
    this._insertMark(qItem);
    exec(cmd, false, err => this.next(err, qItem.id));
  }
  ...
  _processMark(qItem) {
    ...
    const elapse = Date.now() - value.startTime;
    xqItem.pending--;
