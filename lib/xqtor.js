...
class XQtor {
  ...
  execute() {
    ...
    if (vtype === "String") {
      if (value.startsWith(defaults.ANON_SHELL_SIG)) {
        logger.log(`Executing ${chalk.cyan("anonymous")} shell command`);
        console.log(
  ...
  _processTaskObject(qItem) {
    ...
    if (type === "Array") {
      ...
      logger.log(`Executing task ${chalk.cyan(qItem.name)} as shell command`);
      console.log(">>>> executing shell command", itemTask);
    }
