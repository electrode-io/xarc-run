...
class XClap extends EventEmitter {
  ...
  concurrent(...tasks) {
    return this._decorateTasks(defaults.CONCURRENT_SYM, "xclap.concurrent", ...tasks);
  }

  // serial([task1, task2, ...]) or concurrent(task1, task2, ...)
  serial(...tasks) {
    return this._decorateTasks(defaults.SERIAL_SYM, "xclap.serial", ...tasks);
  }

  _decorateTasks(type, name, ...tasks) {
