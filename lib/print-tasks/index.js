...
const printNSTasks = (title, names, tasks, options) => {
  ...
  const printTask = name => {
    ...
    if (tof === "String") {
      console.log(paddedName, chalk.magenta(task));
    } else if (tof === "Array") {
      console.log(paddedName, chalk.green(`${stringify(task)}`));
    } else if (tof === "Object") {
      if (!task.desc) {
        return;
      }
      ...
      if (task.task) {
        console.log(chalk.dim.green(`${spacePad}  tasks: ${stringify(task.task)}`));
      }
      if (task.dep) {
        console.log(chalk.dim.cyan(`${spacePad}  deps: ${stringify(task.dep)}`));
      }
...
function printTasks(tasks, options) {
  const namespaces = Object.keys(tasks).filter(n => n !== defaults.NAMESPACE);
  namespaces.unshift(defaults.NAMESPACE);
