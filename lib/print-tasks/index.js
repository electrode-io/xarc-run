...
const printNSTasks = (title, names, tasks, options) => {
  ...
  const printTask = name => {
    ...
    if (tof === "String") {
      ...
      console.log(paddedName, chalk.green(`${replaceCwd(stringify(task))}`));
    } else if (tof === "Object") {
      if (!task.desc) {
        return;
      }
      ...
      if (task.task) {
        console.log(chalk.dim.green(`${spacePad}  tasks: ${replaceCwd(stringify(task.task))}`));
      }
      if (task.dep) {
        console.log(chalk.dim.cyan(`${spacePad}  deps: ${replaceCwd(stringify(task.dep))}`));
      }
